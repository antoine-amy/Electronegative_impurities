...
class System:
    ...
    def __init__(
        ...
        self.diffusion_constants: Optional[List[float]] = None
        ...
        if setup and material and version:
            ...
            self.volume: Optional[float] = system_props.get("Volume")
            self.area: Optional[float] = system_props.get("Area")
            self.thickness: Optional[float] = system_props.get("Thickness")
    ...
    def get_diff_temp(self) -> None:
        ...
        Returns:
        List[float]: Array of diffusion coefficients at the instance's temperatures.
        """
...
def arrhenius(D0, Ea, T):  # Use system.get_diff_temp() instead
    """
...
def get_impurities_vs_time(data: System) -> List[List[float]]:
    """
    Calculate the impurities over time considering diffusion constants and constraints.

    Args:
    - data (Data): Data object containing initial_impurities, diffusion_constants,
                   time (nested lists of timestamps), thickness, and constraints.

    Returns:
    - List[List[float]]: Nested lists of impurities corresponding to each time segment.
    """
    impurities = []
    # Use the same time segment for all diffusion constants if data.time has only one sublist
    time_segments = (
        [data.time[0] for _ in data.diffusion_constants]
        if len(data.time) == 1
        else data.time
    )
    ...
    for diff_constant, time_segment in zip(data.diffusion_constants, time_segments):
        segment_impurities = []
        current_impurities = (
            data.initial_impurities
        )  # Use a separate variable to avoid mutating the original data

        for timestamp in time_segment:
            current_impurities = solve_diffusion_equation(
                timestamp, diff_constant, data.thickness, current_impurities
            )
            segment_impurities.append(current_impurities)
...
def get_flow_rate_vs_time(data, units="#"):
    ...
    for diff_constant, temp, time_segment in zip(
        data.diffusion_constants, data.temperatures, time_segments
    ):
...
def get_labels(systems, temperature):
    ...
    for system in systems:
        ...
        for temp in temperature:
            label.append(
                rf"{system.version}, $ d= {system.thickness} \, \mathrm{{cm}}, "
                rf"\, T = {temp} \, \mathrm{{K}}, \, E_a = {system.activation_energy}\,\mathrm{{eV}}$"
            )
